...
public class HttpsClientUtil {
    ...
    public String doGet(String url, int times, int sleepSecond, boolean iscookie) throws InterruptedException {
        ...
        while(times-- > 0) {
            ...
            System.out.println("失败重试第 " + ++ errTimes + " 次");
        }
        if(result.length() == 0)
            System.out.println(url + "  失败了，共重试了 " + errTimes + " 次");
    ...
    public String downloadBitFile(String url) {
        String downloadDir = "download/" + DateUtils.getCurrentDateAsString() + "/";
        File saveDir = new File(downloadDir);
        if (!saveDir.exists()) {
            saveDir.mkdirs();
        }
        String downfileFile = url.substring(url.lastIndexOf("/") + 1);
        String extName = downfileFile.substring(downfileFile.indexOf("."));
        CloseableHttpClient client = null;
        CloseableHttpResponse response = null;
        HttpGet httpGet = null;
        try {
